...
async function changeStatus(row, status) {
  ...
  try {
    ...
    const { error } = await supabase.from('purchase_orders').update({ status }).eq('id', row.id)
    if (error) throw error
...
async function handleDelete(row) {
  try {
    await ElMessageBox.confirm('确定删除该采购单吗？', '提示', { type: 'warning' })
...
<template>
  <div class="page-container">
    <div class="page-header">
      <h2>采购管理</h2>
      <el-button type="primary" :icon="Plus" @click="createOrder">新建采购单</el-button>
    </div>
    ...
    <el-table :data="list" v-loading="loading" border stripe>
      ...
      <el-table-column label="操作" width="280" fixed="right">
        <template #default="{ row }">
          ...
          <el-button v-if="row.status === 'confirmed'" link type="success" @click="changeStatus(row, 'received')">入库</el-button>
